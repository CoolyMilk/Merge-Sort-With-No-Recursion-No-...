...
void no_recursion_merge_sort(int *array, int size) {
    int holder[size], marker[size], length = size; // Stuff to simulate 'recursion'
    ...
    while (length > 1) { // I think it's just faster to do it this way instead of calculating how much iterations is required using square root
        ...
        // But it's fixed at the end of the iteration and the number of arrays 
        // at the next pass will be even now
...
they are ones that just sit there)
...
but if it works in the small scale, it should work in the
big scale?
*/
